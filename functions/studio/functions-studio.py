...
text = "launchcode"
print(reverse_characters(text))

    

# 2) The 'split' method does not work on numbers, but we want the function to return a number with all 
# the digits reversed (e.g. 1234 converts to 4321 and NOT the string "4321")
# a) Add an if statement to your reverse_characters function to check the typeof the parameter.
# b - d) If type is ‘string’, return the reversed string as before. If type is ‘number’, convert 
# the parameter to a string, reverse the characters, then convert it back into a number. 
# Return the reversed number.
# e) Be sure to print the result returned by the function to verify that your code works 
# for both strings and numbers. Do this before moving on to the next steps.
...
def reverse_characters(text):
    if type(text) == str:
        ...
        new_text = str(text)
        new_text_reverse = "".join(list(new_text)[::-1])
        num_reverse = int(new_text_reverse)
        return num_reverse
